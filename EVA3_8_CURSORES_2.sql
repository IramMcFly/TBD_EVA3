...
BEGIN
    ...
    DECLARE cur CURSOR FOR SELECT id, nombre, apellido_pat, apellido_mat, fecha_nac 
                           FROM personas 
    ...
    read_loop: LOOP
        ...
        IF _apellido_mat IS NULL THEN
            SET _apellido_mat = 'X';
        END IF;

        UPDATE personas 
        SET clave_rfc = generar_rfc(_nombre, _apellido_pat, _apellido_mat, _fecha_nac) 
        WHERE id = _id;
    END LOOP;
